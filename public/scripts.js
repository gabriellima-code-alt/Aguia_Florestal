...
const MANUAIS_DB = {
        ...
        return new Promise((resolve, reject) => {
            reader.onload = async () => {
                try {
                    ...
                    const tamanho = (pdfFile.size / 1024 / 1024).toFixed(2);
                    ...
                    const resp = await fetch(`${API_BASE}/manuais`, {
                        ...
                        body: JSON.stringify({
                            ...
                            arquivo_pdf: base64,
                            tamanho_mb: parseFloat(tamanho)
                        })
                    });
                    
                    if (!resp.ok) {
                        const data = await resp.json();
                        throw new Error(data.erro || 'Erro ao salvar manual no servidor.');
                    }
                    resolve();
                } catch (e) {
                    reject(e.message);
            ...
            reader.onerror = () => reject("Erro ao ler arquivo PDF.");
    ...
    async obterEquipamentos() {
        try {
            ...
            return data.map(m => ({
                id: m.id_maquina,
                modelo: m.nome_maquina,
                tamanho: m.tamanho_mb + " MB",
                id_db: m.id
            }));
        } catch (e) {
            console.error(e);
    ...
    async obterEquipamento(id) {
        try {
            const resp = await fetch(`${API_BASE}/manuais`);
            const manuais = await resp.json();
            const manual = manuais.find(m => m.id_maquina === id);
            if (!manual) return null;
            
            const respFull = await fetch(`${API_BASE}/manuais?id=${manual.id_db || manual.id}`);
            const fullData = await respFull.json();
            
            const base64Response = await fetch(fullData.arquivo_pdf);
            const blob = await base64Response.blob();
            
            return {
                id: fullData.id_maquina,
                modelo: fullData.nome_maquina,
                pdfFile: blob
            };
        } catch (e) {
            console.error(e);
    ...
    async excluirEquipamento(id) {
        try {
            const respList = await fetch(`${API_BASE}/manuais`);
            const manuais = await respList.json();
            const manual = manuais.find(m => m.id_maquina === id);
            if (!manual) throw new Error('Manual não encontrado.');

            const resp = await fetch(`${API_BASE}/manuais?id=${manual.id_db || manual.id}`, { method: 'DELETE' });
            if (!resp.ok) throw new Error('Erro ao excluir manual.');
        } catch (e) {
            throw e;
        }
    },

    async substituirPDF(id, novoPdfFile) {
        return this.adicionarEquipamento(id, "Atualizado", novoPdfFile);
    }
};
